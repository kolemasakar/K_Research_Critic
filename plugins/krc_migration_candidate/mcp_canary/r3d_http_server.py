...
def handle_r3d_http_request(
    ...
    *,
    ...
    if clean_path == HEALTH_PATH and method == "GET":
        return _json_response(HTTPStatus.OK, r3d_health(GLOBAL_R3D_PROBE_STATE))

    if clean_path != MCP_PATH:
        return _json_response(HTTPStatus.NOT_FOUND, {"status": "not_found"})
    if config.surface != R3D_SURFACE:
        return _json_response(HTTPStatus.SERVICE_UNAVAILABLE, {"status": "r3d_surface_required"})
    if config.auth_mode != "oauth":
        return _json_response(HTTPStatus.SERVICE_UNAVAILABLE, {"status": "r3d_requires_oauth"})
    ...
    if modern:
        validation = _validate_modern_headers(
            decoded,
            normalized,
            allowed_tool_names=R3D_TOOL_NAME_SET,
        )
        if validation:
...
class R3DRequestHandler(BaseHTTPRequestHandler):
...
class R3DHTTPServer(ThreadingHTTPServer):
    daemon_threads = True
...
def main() -> None:
    ...
    server = R3DHTTPServer((host, port), R3DRequestHandler)
