...
class StateMachine:
    ...
    ALLOWED_TRANSITIONS: dict[TaskStatus, frozenset[TaskStatus]] = {
        ...
        TaskStatus.PROFILE_GENERATING: frozenset(
            {TaskStatus.PROFILE_REVIEW_REQUIRED, TaskStatus.FAILED}
        ),
        TaskStatus.PROFILE_REVIEW_REQUIRED: frozenset(
            {
        ...
        TaskStatus.PROFILE_APPROVED: frozenset(
            {TaskStatus.RESEARCHING, TaskStatus.PROFILE_GENERATING, TaskStatus.FAILED}
        ),
        TaskStatus.RESEARCHING: frozenset(
            {TaskStatus.DRAFT_READY, TaskStatus.PROFILE_GENERATING, TaskStatus.FAILED}
        ),
        TaskStatus.DRAFT_READY: frozenset(
            {TaskStatus.REVIEWING, TaskStatus.PROFILE_GENERATING, TaskStatus.FAILED}
        ),
        ...
        TaskStatus.REVISE_REQUIRED: frozenset(
            {
        ...
        TaskStatus.APPROVED: frozenset(
            {TaskStatus.FINALIZING, TaskStatus.PROFILE_GENERATING, TaskStatus.FAILED}
        ),
        ...
        TaskStatus.MAX_ITERATIONS_REACHED: frozenset(
            {TaskStatus.COMPLETED_WITH_LIMITATIONS, TaskStatus.FAILED}
        ),
    ...
    def transition(self, task: Task, to_state: TaskStatus) -> tuple[TaskStatus, TaskStatus]:
        ...
        if from_state in self.TERMINAL_STATES:
            raise InvalidStateTransitionError(
                f"Terminal state {from_state} cannot transition to {to_state}"
            )
