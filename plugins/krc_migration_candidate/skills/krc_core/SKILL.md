...
Canonical source: `prompts/GPT_STORE_INSTRUCTIONS.md`.

The text between the markers below is an exact migration snapshot of the accepted Core instructions. Repository tests require byte-for-byte parity with the canonical source after newline normalization. Do not edit the snapshot independently; update the canonical Core first and then regenerate this candidate.

<!-- BEGIN KRC CORE EXACT SNAPSHOT -->
