...
R3E4_PREFLIGHT_PROBE_ENV = "KRC_R3E4_PREFLIGHT_PROBE_URL"
R3E4_REPLAY_PROBE_ENV = "KRC_R3E4_REPLAY_PROBE_JOB_ID"
R3E4_CONFIRMATION_PROBE_ONLY_ENV = "KRC_R3E4_CONFIRMATION_PROBE_ONLY"
_CONFIRMATION_PROBE_INVOCATIONS = 0


def _confirmation_probe_enabled() -> bool:
    return os.getenv(R3E4_CONFIRMATION_PROBE_ONLY_ENV, "").strip().lower() == "true"
    ...
    while True:
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "krc-media-mcp-r3e4-warmup",
            },
            method="GET",
        )
        try:
...
def handle_r3e4_http_request(
    ...
    *,
    ...
    if clean_path == HEALTH_PATH and method == "GET":
        health = r3e4_health(_binding(config))
    ...
    if config.surface != R3E4_SURFACE:
        return _json_response(
            HTTPStatus.SERVICE_UNAVAILABLE,
            {"status": "r3e4_surface_required"},
        )
    if config.auth_mode != "oauth":
        return _json_response(
            HTTPStatus.SERVICE_UNAVAILABLE,
            {"status": "r3e4_requires_oauth"},
        )
    ...
    if modern:
        validation = _validate_modern_headers(
            decoded,
            normalized,
            allowed_tool_names=R3E4_TOOL_NAME_SET,
        )
        if validation:
    ...
    if (
        ...
        and decoded["params"].get("name") == "media_telegram_start"
    ):
...
class R3E4RequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle(self) -> None:
        ...
        response = handle_r3e4_http_request(
            self.command,
            self.path,
...
class R3E4HTTPServer(ThreadingHTTPServer):
    daemon_threads = True
...
def run_r3e4_record_replay_probe(
    config: HttpConfig,
    job_id: str,
) -> dict[str, object]:
    """Read durable Telegram job status and segments without provider work."""

    summary: dict[str, object] = {
        "event": "r3e4_record_replay_probe",
        "status": "fail",
        "job_id": job_id,
    ...
    status_response = dispatch_r3e4(status_message, _binding(config))
    ...
    segments_response = dispatch_r3e4(segments_message, _binding(config))
...
def run_r3e4_preflight_lookup_probe(
    config: HttpConfig,
    source_url: str,
) -> dict[str, object]:
    """Read Telegram free-route capabilities and durable lookup; never call start."""

    summary: dict[str, object] = {
        "event": "r3e4_telegram_preflight_lookup_probe",
        "status": "fail",
        "provider_work_started": False,
        "start_called": False,
    }
    ...
    expected = (
        capabilities.get("telegram_public_retrieval") is True
        and capabilities.get("telegram_retrieval_provider") == "telegram_public_web"
        and capabilities.get("telegram_retrieval_credits") == 0
        and capabilities.get("telegram_stt_provider") == "assemblyai"
        and capabilities.get("telegram_stt_configured") is True
    )
    if not expected:
        summary["reason"] = "capabilities_contract_mismatch"
        return summary
    summary["preflight"] = "pass"
    summary["provider"] = "telegram_public_web"
...
def main() -> None:
    ...
    probe_url = os.getenv(R3E4_PREFLIGHT_PROBE_ENV, "").strip()
    if probe_url:
        print(
            json.dumps(
                run_r3e4_preflight_lookup_probe(config, probe_url),
                separators=(",", ":"),
                sort_keys=True,
            ),
            flush=True,
    ...
    replay_job_id = os.getenv(R3E4_REPLAY_PROBE_ENV, "").strip()
    if replay_job_id:
        print(
            json.dumps(
                run_r3e4_record_replay_probe(config, replay_job_id),
                separators=(",", ":"),
                sort_keys=True,
            ),
            flush=True,
    ...
    server = R3E4HTTPServer((host, port), R3E4RequestHandler)
